...
const DNA_AXES = [
  ...
  { key: 'sharing' as const, label: 'Sharing', color: '#888888' },
];
...
function ScoreCircle({ score }: { score: number }) {
  ...
  if (score >= 90) { color = '#888888'; bg = 'rgba(16,185,129,0.1)'; }
  else if (score >= 80) { color = '#FDBA2D'; bg = 'rgba(253,186,45,0.1)'; }
  else if (score >= 70) { color = '#888888'; bg = 'rgba(59,130,246,0.1)'; }
  else if (score >= 50) { color = '#888888'; bg = 'rgba(139,92,246,0.1)'; }
  ...
  return (
    <button
      ...
      className={`flex items-center gap-2 px-4 py-2.5 rounded-t-lg text-sm font-medium transition-colors border-b-2 ${
        active
          ? 'text-[#888888] border-[#888888] bg-[#0f0f0f]'
    ...
    <div className="flex flex-col items-center">
      <div className="relative w-36 h-36">
        <svg className="w-full h-full -rotate-90" viewBox="0 0 120 120">
          {/* Background circle */}
          <circle cx="60" cy="60" r={radius} fill="none" stroke="#0f0f0f" strokeWidth="8" />
          {/* Score arc */}
          <circle
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          ...
          <span className="text-[10px] text-[#a0a0a0] uppercase tracking-wider">Viral Score</span>
        </div>
      </div>
      <span className="mt-2 text-xs font-medium text-[#a0a0a0]">
    ...
    <div className="flex items-center justify-center">
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="overflow-visible">
        ...
        {gridLines.map((points, i) => (
          <polygon key={i} points={points} fill="none" stroke="#0f0f0f" strokeWidth="1" />
        ))}
        ...
        {points.map((p, i) => (
          <g key={i}>
            <circle cx={p.x} cy={p.y} r="4" fill={p.axis.color} stroke="#0a0a0a" strokeWidth="2" />
          </g>
        ))}
        ...
        {DNA_AXES.map((axis, i) => {
          ...
          return (
            <g key={axis.key}>
              <text x={x} y={y - 6} textAnchor={anchor} fill="#a0a0a0" fontSize="9" fontWeight="500">
                {axis.label}
              </text>
        ...
        <div className="px-4 sm:px-5 py-4 border-b border-[#1A1A1A]">
          <div className="flex items-center gap-3 mb-3">
            <div className="p-2 rounded-lg bg-[rgba(255,255,255,0.06)]">
              <Stethoscope className="w-5 h-5 text-[#888888]" />
            </div>
            <div>
              <h2 className="text-base font-bold text-[#FFFFFF]">Viral Autopsy</h2>
              <p className="text-xs text-[#a0a0a0] mt-0.5">Deep-dive analysis of what makes any YouTube video go viral.</p>
            </div>
          </div>
          <div className="flex rounded-full bg-[#0a0a0a] border border-[#1A1A1A] overflow-hidden">
            ...
            <button
              ...
              className="px-5 h-11 rounded-full bg-[#888888] text-[#0a0a0a] text-sm font-bold hover:bg-[#0A9A74] transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 shrink-0"
            >
      ...
      {loading && (
        <div className="rounded-lg bg-[#0f0f0f] border border-[rgba(255,255,255,0.06)] p-8">
          <div className="flex flex-col items-center justify-center space-y-4">
            <div className="w-20 h-20 rounded-full border-4 border-[rgba(255,255,255,0.06)] border-t-[#888888] animate-spin" />
            <div className="text-center">
              <p className="text-sm font-semibold text-[#FFFFFF]">Analyzing video...</p>
              <p className="text-xs text-[#a0a0a0] mt-1">Breaking down virality factors, hook structure, and algorithm signals</p>
            </div>
          </div>
        </div>
      )}
      ...
      {!loading && analyzed && result && (
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <div className="rounded-lg bg-[#0f0f0f] border border-[rgba(255,255,255,0.06)] p-6">
              <h3 className="text-sm font-semibold text-[#FFFFFF] mb-4 flex items-center gap-2">
                <Brain className="w-4 h-4 text-[#888888]" />
                Virality DNA
              </h3>
          ...
          <div className="rounded-lg bg-[#0f0f0f] border border-[rgba(255,255,255,0.06)] p-5">
            ...
            <p className="text-sm text-[#a0a0a0] leading-relaxed">{result.thumbnailPsychology}</p>
          </div>
            ...
            <div className="space-y-2">
              {result.titleFormula.elements.map((element, i) => (
                <div key={i} className="flex items-start gap-2">
                  <ArrowRight className="w-3.5 h-3.5 text-[#888888] mt-0.5 shrink-0" />
                  <p className="text-sm text-[#a0a0a0]">{element}</p>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
              ...
              <div className="p-3 rounded-lg bg-[#0a0a0a] border border-[#1A1A1A]">
                <div className="flex items-center gap-2 mb-1.5">
                  ...
                  <span className="text-[10px] uppercase tracking-wider font-medium text-[#a0a0a0]">Retention</span>
                </div>
                <p className="text-xs text-[#a0a0a0] leading-relaxed">{result.algorithmSignals.retention}</p>
              </div>
              <div className="p-3 rounded-lg bg-[#0a0a0a] border border-[#1A1A1A]">
                <div className="flex items-center gap-2 mb-1.5">
                  <TrendingUp className="w-3.5 h-3.5 text-[#888888]" />
                  <span className="text-[10px] uppercase tracking-wider font-medium text-[#a0a0a0]">CTR Velocity</span>
                </div>
                <p className="text-xs text-[#a0a0a0] leading-relaxed">{result.algorithmSignals.ctrVelocity}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {result.shareTriggers.map((trigger, i) => (
                <div key={i} className="p-3 rounded-lg bg-[#0a0a0a] border border-[#1A1A1A]">
                  ...
                  <p className="text-[11px] text-[#a0a0a0] leading-relaxed">{trigger.description}</p>
                  {/* Mini bar */}
      ...
      {!loading && !analyzed && (
        <div className="flex flex-col items-center justify-center py-16">
          <div className="w-16 h-16 rounded-2xl bg-[rgba(255,255,255,0.06)] border border-[rgba(255,255,255,0.06)] flex items-center justify-center mb-4">
            <Stethoscope className="w-8 h-8 text-[#888888]" />
          </div>
          <h3 className="text-base font-semibold text-[#FFFFFF] mb-1">Viral Autopsy</h3>
          <p className="text-sm text-[#a0a0a0] max-w-xs text-center">Paste any YouTube URL to get a deep-dive analysis of what makes it viral.</p>
        </div>
      )}
    </div>
  );
            ...
            <div className="rounded-lg bg-[#0f0f0f] border border-[rgba(255,255,255,0.06)] overflow-hidden">
              <div className="flex items-center justify-between px-4 py-3 border-b border-[rgba(255,255,255,0.06)]">
                <h3 className="text-sm font-semibold text-[#FFFFFF]">
                  Score Breakdown
                  {threshold !== 'all' && (
                    <span className="text-xs text-[#a0a0a0] ml-2">({threshold}+)</span>
                  )}
                </h3>
              ...
              <div className="divide-y divide-[#1A1A1A] max-h-[600px] overflow-y-auto">
                {sorted.length === 0 ? (
                  <div className="py-12 text-center">
                    <Zap className="w-8 h-8 text-[#666666] mx-auto mb-2" />
                    <p className="text-sm text-[#a0a0a0]">No videos match this threshold</p>
                  </div>
                ) : (
                  sorted.map((item) => (
                    <div
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 mt-1">
                          <span className="text-[11px] text-[#a0a0a0]">{item.channelTitle}</span>
                      ...
                      <div className="hidden sm:flex flex-col items-end gap-1 shrink-0">
                        <span className="text-xs font-medium text-[#888888]">↑ {item.growthRate}%</span>
                        <span className="text-[11px] text-[#a0a0a0]">{item.engagementRate}% engage</span>
                      </div>
