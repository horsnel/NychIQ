...
Return ONLY the JSON object.`;
  ...
  return (
    <div className="space-y-5 animate-fade-in-up">
              ...
              {result.thumbnailEvolution.map((phase, i) => (
                <div key={i} className="flex gap-3">
                  ...
                  <div className="flex-1 pb-4">
                    ...
                    <p className="text-xs text-[#a0a0a0] mt-0.5">{phase.description}</p>
                    <p className="text-xs text-[#888888] mt-1">Impact: {phase.impact}</p>
                  </div>
                </div>
              ))}
            </div>
              ...
              {result.titlePatterns.map((tp, i) => (
                <div key={i} className="p-3 rounded-md bg-[#0a0a0a] border border-[#1A1A1A]">
                  ...
                  <div className="flex items-center gap-3 mt-1.5">
                    ...
                    <span className="text-[10px] text-[#888888]">{tp.performance}</span>
                  </div>
                </div>
              ))}
            </div>
              ...
              {result.uploadTrends.map((ut, i) => (
                <div key={i} className="flex items-start gap-2 p-3 rounded-md bg-[rgba(255,255,255,0.06)] border border-[rgba(255,255,255,0.06)]">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[#888888] mt-0.5">Impact: {ut.impact}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      {searched && !loading && <div className="text-center text-[11px] text-[#666666]">Cost: {TOKEN_COSTS['history-intel']} tokens per analysis</div>}
    </div>
  );
}
