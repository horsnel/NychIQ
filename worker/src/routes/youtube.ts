 ...
 * Fallback chain: Piped → Invidious → YouTube Data API v3 (key rotation)
...
// ── Normalizers: convert Piped/Invidious → YouTube API v3 format ──
    ...
    const result = await withFallback<any>([
      ...
      // 2. Invidious
      {
        ...
        fn: async () => {
          ...
          if (regionCode) params.set('region', regionCode);
          const res = await fetch(`${INVIDIOUS_INSTANCES[0]}/api/v1/search?${params}`);
      ...
      // 3. YouTube Data API v3
      {
        ...
        fn: async () => {
          ...
          const params = new URLSearchParams({
            part: 'snippet', q, type, maxResults,
          });
      ...
      ...(id ? [{
        ...
        fn: async () => {
          ...
          const videos = await Promise.all(
            ids.map(vid => fetch(`${INVIDIOUS_INSTANCES[0]}/api/v1/videos/${vid}`).then(r => r.json()).catch(() => null))
          );
  ...
  try {
    const result = await withFallback<any>([
      ...
      // 2. Invidious trending
      {
        name: 'invidious',
        fn: async () => {
          const res = await fetch(`${INVIDIOUS_INSTANCES[0]}/api/v1/trending?region=${regionCode}`);
          if (!res.ok) throw new Error(`Invidious ${res.status}`);
          const data: any = await res.json();
    ...
    const result = await withFallback<any>([
      // 1. Invidious
      {
        ...
        fn: async () => {
          ...
          const res = await fetch(`${INVIDIOUS_INSTANCES[0]}/api/v1/channels/${lookup}`);
          if (!res.ok) throw new Error(`Invidious ${res.status}`);
          const data: any = await res.json();
  ...
  try {
    const result = await withFallback<any>([
      ...
      // 2. Invidious captions
      {
        name: 'invidious',
        fn: async () => {
          const res = await fetch(`${INVIDIOUS_INSTANCES[0]}/api/v1/captions/${videoId}`);
          if (!res.ok) throw new Error(`Invidious ${res.status}`);
          const data: any = await res.json();
          ...
          const captionRes = await fetch(`${INVIDIOUS_INSTANCES[0]}/api/v1/captions/${videoId}?label=${encodeURIComponent(track.label)}`);
