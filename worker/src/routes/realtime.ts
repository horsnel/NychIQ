 ...
 * WebSocket upgrade and HTTP publish endpoints.
 * Backend: Durable Objects (always available) + Ably/Soketi/Supabase RT fallbacks.
 */
...
/**
 * GET /api/realtime/ws/:channel — WebSocket upgrade via Durable Objects
 */
realtimeRoutes.get('/ws/:channel', async (c) => {
...
/**
 * GET /api/realtime/endpoint — Get the best realtime endpoint for a channel
 * Params: channel
 */
...
realtimeRoutes.post('/publish', async (c) => {
  try {
    ...
    return c.json({ error: 'All realtime providers failed' }, 500);
  } catch (err: any) {
